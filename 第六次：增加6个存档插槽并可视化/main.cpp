...
void StartRanDomizedGame()
{
    ...
    Button* but[7], *But[7],*bb;
    Button *mpx[16],*mpy[16];
    bb=CreateButton(WIN_W/2,640,60,30,"确定");
    for(int i=1;i<=6;i++)
    {
        string ch="";
        char cc;
...
void Edit()
{
    ...
    int cc;
    std::cout<<"是否读取地图存档继续编辑？1：是  其他：否"<<endl;
    cin>>cc;
    if(cc==1)
    {
        Map_Load();
    ...
    while(n>18||m>18||n<=0||m<=0)  {std::cout<<"请重试"<<endl;  cin>>n>>m;}
    int t1,t2,t3;
    std::cout<<"输入玩家个数和人机个数，两数之和不大于6且大于1，人机个数可以为0，玩家个数不能为0"<<endl;
    while(h_p+a_p>=7||h_p<=0||a_p<0||h_p+a_p<=1)  {std::cout<<"请重试"<<endl;cin>>h_p>>a_p;}
    std::cout<<"输入地图样式，格式为：x y t，其中x,y表示坐标。t:0表示空地，-1表示不可通过，1-6表示国家，系统不检查是否连通"<<endl;
    while(1)
    {
        std::cout<<"输入114查看当前地图，输入514退出，,输入1919查看帮助，输入810生成一张随机地图。否则正常输入x,y,z三个数"<<endl;
        cin>>t1;
        if(t1==114)
        {
            for(int i=1;i<=n;i++)
            {
                for (int j=1;j<=m;j++)
                {
                    if(newma[i][j]==-1)  std::cout<<-1<<' ';
                    else std::cout<<' '<<newma[i][j]<<' ';
                }
                std::cout<<endl;
            }
        }
        else  if(t1==514)
        {
            ...
            std::cout<<"输入地图样式，格式为：x y t，其中x,y表示坐标。t:0表示空地，-1表示不可通过，1-6表示国家"<<endl
        ...
        else
        {
            ...
            else  std::cout<<"请重试"<<endl;
        }
    }
    std::cout<<"导出请输入1，退出（不保存）请输入2"<<endl;
    t1=0;
    while(t1!=1&&t1!=2)
    {
        cin>>t1;
