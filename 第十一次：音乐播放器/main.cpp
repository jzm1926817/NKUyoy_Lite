...
void HumanPlay(int pnum)//人类玩家界面
{
    ...
    Button* butMusic = CreateButton(0, WIN_H-150, 100, 25, "音乐播放器");
    ExMessage msg;
    while(1)
    {
        if (peekmessage(&msg)) {
...
void Game_Start()
{
    ...
    {
        MessageBox(GetHWnd(),"地图未正确加载，可能的原因有：该存档不正确，该存档已损坏或编辑器中制作的存档不正确。","游戏提示",MB_OK);
    }
    closegraph();
    Menu();
}
...
void MusicPlay(int t)
{
    ...
    switch(t)
    {
        ...
        case 7:mciSendString("open ./res/凯南开大学の小曲.wav alias bk",NULL,0,NULL); break;
        case 8:mciSendString("open ./res/只因你太美の小曲.wav alias bk",NULL,0,NULL); break;
        case 9:mciSendString("open C:/Users/1/Music/自定义1.wav alias bk",NULL,0,NULL); break;
        case 10:mciSendString("open C:/Users/1/Music/自定义2.wav alias bk",NULL,0,NULL); break;
        case 11:mciSendString("stop bk", NULL, 0, NULL);  mciSendString("close bk", NULL, 0, NULL);  break;
        default:  break;
    }
    mciSendString("play bk notify",0,0,0);
}
void MusicPlayMenu()
{
    IMAGE img_bk;
