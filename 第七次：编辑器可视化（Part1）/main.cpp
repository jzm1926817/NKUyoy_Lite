...
{   
    ...
    Button* butarmy[4];
    Button* butcolor[10];
    Button* butRandom=CreateButton(0, WIN_H-200, 100, 25, "随机生成");
    Button* butLoad=CreateButton(0, WIN_H-150, 100, 25, "读取地图");
    Button* butSave=CreateButton(0, WIN_H-100, 100, 25, "保存并退出");
    Button* butReturn=CreateButton(0, WIN_H-50, 100, 25, "不保存退出");
    string ch,ch2;
    for(int i=0;i<=7;i++)
    {
        ch=to_string(i-1);
    ...
    {
        ch="兵种等级"+to_string(i);
        butarmy[i]=CreateButton(200+100*i,10,75,25,ch.c_str());
    }
    int curgrade=0;
    int curcolor=0;
    ExMessage msg;
    ...
    while(1)
    {
        if (peekmessage(&msg)) {
            for(int i=0;i<=7;i++)
            {
                if(UpdataButton(msg,butarmy[i]))  curgrade=i;
                if(i<=3&&UpdataButton(msg,butcolor[i]))   curcolor=i;
            }
        ...
        if(curcolor==-1)  setfillcolor(_Colors[10]);
        fillrectangle(250,30,275,55);
