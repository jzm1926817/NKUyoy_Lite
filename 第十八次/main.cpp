...
//游戏规则判定相关
void NewConj(int nx,int ny,int pnum);//计算截断
void MaintCounting(int pnum);//计算维护费
int isOurLand(int x0,int y0,int pnum);//判断土地归属
void ConJudge();//连通块判定
void CashCounting(int pnum);//结算金币
void IniCanMove(int pnum);//判断移动是否可以进行
int IsAnArmy(int x0,int y0);//判断当地有没有军队
...
void txtprintChessBoard()//调试用
{
    ...
    for(int i=1;i<=n;i++)
    {
        for(int j=1;j<=m;j++)
        {
            if(q[ma[i][j]][r[i][j]]!=-1)  std::cout<<' ';
            std::cout<<q[ma[i][j]][r[i][j]];
            std::cout<<' ';
        }
        std::cout<<endl;
    }
    ...
    string hlp;
       ...
       hlp+="1.不同颜色代表不同国家的国土，获胜条件为其他所有国家的国土格数均为0；\n";
        ...
        hlp+="游戏模式：\n";
            ...
            hlp+="3.预设：本游戏有作者预设的几张地图，但是不确保平衡性。注意：使用预设地图请现在编辑器确定游戏人数然后再读取地图存档进行游戏。\n\n";
